...
const vocabSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchVocabs.fulfilled, (state, action) => { state.loading = false; state.vocabs = action.payload; })
      .addCase(fetchVocabs.rejected, (state, action) => { state.loading = false; state.error = action.payload as string; })

      .addCase(addVocab.fulfilled, (state, action) => { state.vocabs.push(action.payload); })
